...
namespace Jaunty.Tests.Postgres.IntegrationTests
{
	public class Northwind : IDisposable
	{
		...
		private void CreateDatabase()
		{
			...
			string connectionString = $"server=localhost;port=5432;database=postgres;user id=dotnet;password=csharp-rocks!";
			Connection = new NpgsqlConnection(connectionString);
			Connection.Execute($"CREATE DATABASE {Name};");
			connectionString = $"server=localhost;port=5432;database={Name};user id=dotnet;password=csharp-rocks!";
		...
		{
			string northwindDir = Path.Combine(Directory.GetCurrentDirectory(), @"..\..\..\..\..\..\", @"databases\src\Northwind\Postgres");
